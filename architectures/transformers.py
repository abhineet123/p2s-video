...
def get_ar_mask(seq_len, dtype=tf.float32):
    """Get autoregressive causal mask so the model cannot attend to the future.
...
def top_logits(logits: tf.Tensor,
    ...
    Args:
      ...
      mask: a value that's used to replace logits that don't satisfy the
        keep conditions.
...
class FeedForwardLayer(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
    ...
    def __init__(self,
        ...
        """
        dim_mlp is the output size for dense1 while dim_att is that for dense2
        tf.keras.layers.Dense does not require input size to be specified - it is inferred from the first call 
