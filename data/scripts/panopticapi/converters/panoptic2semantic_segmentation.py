...
def extract_semantic_single_core(proc_id,
    ...
    for working_idx, annotation in enumerate(annotations_set):
        ...
        for segm_info in annotation['segments_info']:
            ...
            cat_id_mapped = category_id_map[cat_id]
            if save_as_png:
                semantic[mask] = cat_id_mapped
            else:
                RLE = COCOmask.encode(np.asfortranarray(mask.astype('uint8')))
                RLE['counts'] = RLE['counts'].decode('utf8')
                RLE_per_category[cat_id_mapped].append(RLE)
...
def extract_semantic(
    ...
    if root_dir:
        if segmentations_folder:
            segmentations_folder = linux_path(root_dir, segmentations_folder)
        if input_json_file:
            input_json_file = linux_path(root_dir, input_json_file)
        if semantic_seg_folder:
            semantic_seg_folder = linux_path(root_dir, semantic_seg_folder)
        if output_json_file:
            output_json_file = linux_path(root_dir, output_json_file)
    ...
    """
    coco has non-consecutive class IDs for some mind-bogglingly annoying and inane reasons
    all these foul datasets seem to be going out of their way to make integration difficult 
    with multi-dataset systems and cause endless time-waste for no good reason
    """
    category_id_map = {
        category['id']: class_id + 1 for class_id, category in enumerate(categories_list)
    }

    # annotations_coco_semantic_seg = extract_semantic_single_core(
    #     0, annotations, segmentations_folder, output_json_file, semantic_seg_folder,
    #     categories, category_id_map, save_as_png, things_other)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--categories_json_file', type=str,
                        ...
                        default='data/scripts/panopticapi/panoptic_coco_categories.json')
